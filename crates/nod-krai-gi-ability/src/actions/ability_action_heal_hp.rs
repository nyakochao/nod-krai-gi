...
use nod_krai_gi_entity::common::{
    EntityById, FightProperties, InstancedAbilities, InstancedModifiers, OwnerProtocolEntityID,
};
            ...
            Some(owner_entity) => {
                let caster_heal_add;
                {
                    let Ok(caster_props) = fight_props_query.get(owner_entity) else {
                        if GAME_SERVER_CONFIG.plugin.ability_log {
                            tracing::debug!(
                                "[ability_action_heal_hp_event] owner_entity props not found"
                            );
                        }
                        continue;
                    };
                ...
                );
                {
                    ...
                    let Ok(target_props) = fight_props_query.get(target_entity) else {
                        if GAME_SERVER_CONFIG.plugin.ability_log {
                            tracing::debug!(
                                "[ability_action_heal_hp_event] target_entity props not found"
                            );
                        }
                        continue;
                    };
