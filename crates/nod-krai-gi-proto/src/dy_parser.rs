...
fn update_proto(name: &str) {
    ...
    let version_config_file_path = std::path::Path::new("./assets/proto/")
        .join(name)
        .join("version_config.json");

    if !proto_file_path.exists()
        || !protocol_file_path.exists()
        || !replace_file_path.exists()
        || !version_config_file_path.exists()
    {
        ...
        .parse_and_typecheck()
    {
        Ok(parsed) => {
            ...
            let mut version_config = VersionConfig::default();
            version_config.ty_value = 24;
            ...
            if version_config_file_path.exists() {
                version_config =
                    serde_json::from_slice(&*std::fs::read(version_config_file_path).unwrap())
                        .unwrap();
            };
...
fn update_cmd(name: &str) {
        ...
        .contains_key(&name.to_string())
    {
        ...
        let version_config = MULTI_VERSION_PROTOCOL
            .get()
            .unwrap()
            .get(&name.to_string())
            .unwrap()
...
pub fn get_name_by_cmd_id_version(version: &str, cmd_id: u16) -> Option<String> {
    match MULTI_VERSION_PROTOCOL.get().unwrap().get(version) {
        ...
        Some(data) => match data.cmd_id_map_re.get(&cmd_id) {
            None => {
...
pub fn get_cmd_id_by_name_version(version: &str, message_name: &str) -> Option<u16> {
    match MULTI_VERSION_PROTOCOL.get().unwrap().get(version) {
        ...
        Some(data) => match data.cmd_id_map.get(message_name) {
            None => {
                tracing::error!(
...
pub fn replace_out_u32(version: &str, field_name: &str, ov: u32) -> u32 {
    match MULTI_VERSION_PROTOCOL.get().unwrap().get(version) {
        ...
        Some(data) => match data.replace_value_map.get(field_name) {
            None => ov,
            Some(config) => {
                let result;
